...
from src.megapose.lib3d.transform_ops import invert_transform_matrices
...
def worker_loop(
    ...
    while True:
        ...
        output = RenderOutput(
            ...
            rgb=torch.from_numpy(renderings_.rgb),  # .share_memory_(),
            normals=torch.from_numpy(renderings_.normals)  # .share_memory_()
            if render_args.render_normals
            else None,
            depth=torch.from_numpy(renderings_.depth)  # .share_memory_()
    ...
    def render(
        ...
        if render_normals:
            ...
            try:
                ...
                normal_shape = list_normals[0].shape
                if normal_shape[2] != 3:
                    for normal in list_normals:
                        if len(normal.shape) == 3 and normal.shape[2] == 3:
                            normal_shape = normal.shape
                for idx, normal in enumerate(list_normals):
                    if normal.shape != normal_shape:
                        normal = normal.repeat(1, 1, 3)
                        list_normals[idx] = normal
                normals = torch.stack(list_normals).pin_memory().cuda(non_blocking=True)
                logger.info(f"Fix normal shape: {normals.shape}")
