...
# MegaPose
from src.megapose.config import SHAPENET_DIR
from src.megapose.datasets.datasets_cfg import make_object_dataset
from src.megapose.lib3d.rigid_mesh_database import as_mesh
...
def make_ply_scaled(mesh_path, scale=SCALE):
    ...
    new_mesh_path = new_mesh_path.replace(
        "models_orig", MODELS_DIR_TEMPLATE.format(scale=scale)
    )
    mesh = trimesh.load(
        str(mesh_path), skip_materials=True, process=False, maintain_order=True
    )
    mesh = as_mesh(mesh)
