...
def deepim_crops(
    images, obs_boxes, K, TCO_pred, O_vertices, output_size=None, lamb=1.4
):
    ...
    boxes = deepim_boxes(
        rend_center_uv, obs_boxes, rend_boxes, im_size=(h, w), lamb=lamb
    )
    boxes = torch.cat(
...
def deepim_crops_robust(
    ...
    rend_center_uv = project_points_robust(
        torch.zeros(batch_size, 1, 3).to(device), K, TCR
    )
    boxes = deepim_boxes(
        rend_center_uv, obs_boxes, rend_boxes, im_size=(h, w), lamb=lamb
    )
