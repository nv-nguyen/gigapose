# credit: https://github.com/thodan/bop_toolkit/blob/master/bop_toolkit_lib/dataset/bop_webdataset.py
"""
...
def load_pred_mask_data(
    ...
    image_data["mask"] = bop_imagewise.io_load_masks(
        _load("mask.json", read=False), instance_ids=None
    )
    box_data = _load("box.npy", read=True)
    image_data["box"] = np.frombuffer(box_data, dtype=np.int64).reshape(-1, 4)
    return image_data
